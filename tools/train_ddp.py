...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--save-path", default="./lenet_cifar10_ddp.pt", help="Checkpoint output path")
...
def main() -> None:
    ...
    dataset = datasets.CIFAR10(root=args.data_dir, train=True, download=(rank == 0), transform=transform)
    dist.barrier()
    if rank != 0:
    ...
    ddp_model = DistributedDataParallel(model, device_ids=[local_rank] if torch.cuda.is_available() else None)
